...
clumped.filename <- "out/1_5r_500nq_1pctm_0.1h2_0.01ve.clumped"
scan.filename <- "out/1_5r_500nq_1pctm_0.1h2_0.01ve.scan.tsv"
ld.filename <- "out/1_5r_500nq_1pctm_0.1h2_0.01ve.ld"
out.filename <- "foo"
...
for (j in seq_along(nsnp)) {
  ...
  if (nsnp[j] > 1) {
    ...
    n <- length(out2$beta_hat_a[[j]])
    df1 <- with(out2, data.frame(SNP=1:n,
                                 coef=c(beta_hat_a[[j]], beta_hat_b[[j]]),
                                 se=c(se_a[[j]], se_b[[j]]),
                                 data=rep(c("eQTL","GWAS"),each=n),
                                 est="summary"))
    df2 <- with(fit, data.frame(SNP=1:n,
                                coef=c(beta_hat_a, beta_hat_b),
                                se=c(sd_a, sd_b),
                                data=rep(c("eQTL","GWAS"),each=n),
                                est="mrlocus"))
    df3 <- with(out1$sum_stat[[j]], data.frame(SNP=1:n,
                                               coef=c(eqtl.true, gwas.true),
#                                               coef=-1*c(eqtl.true, gwas.true),
                                               se=0,
                                               data=rep(c("eQTL","GWAS"),each=n),
                                               est="true"))
    df <- rbind(df1, df2, df3)
    save(df, file=paste0("sim1_seed5_cluster",j,".rda"))
