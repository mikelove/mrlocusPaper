...
signal clusters (this term used for clumps after they are defined), in
which the pair-wise $r^2$ of the index eSNP is greater than
...
cluster, then the second cluster, etc.
...
\subsubsection{Colocalization with eCAVIAR}
...
For eCAVIAR, the SNP with the largest colocalization posterior
probability (CLPP) is selected from each signal cluster among those
SNPs which are valid instruments, i.e. with an absolute value of
z-score as large as the value used for PLINK clumping \citep{plink}.
TODO second round trimming
MRLocus also has the non-default option to perform model-based clustering using 
an EM algorithm to determine how many SNPs per nearly-LD-independent
