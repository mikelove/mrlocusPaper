...
eQTL and GWAS summary statistics arise from the same causal variant,
...
The second step of MRLocus is to estimate the slope using the
posterior mean values from the colocalization step. For each
LD-independent signal cluster, MRLocus extracts one SNP, based on the
largest posterior mean value for $\beta_{i,j}^A$ (prioritizing the
mediator trait for variant selection). MRLocus also has the
non-default option to perform model-based clustering using a fixed
number of EM steps to determine how many SNPs per LD-independent
signal cluster to pass to the slope fitting step \citep{mclust}.

By default, $J$ SNPs are passed from the colocalization step to the
slope fitting step, which represent ideally the causal SNP from each
LD-independent signal cluster. The posterior mean for
$\beta_{i,j}^A$ and $\beta_{i,j}^B$ for these SNPs is then provided to
the slope fitting step as $\widehat{\beta}_j^A$ and
$\widehat{\beta}_j^B$ which is modeled again as normally distributed
variable with standard deviation according to the original standard
errors $\se(\widehat{\beta}_{i,j}^A)$ and
...
accurately estimate the uncertainty on the slope, giving our
uncertainty involved in estimating the coefficients.
...
mean of $\alpha$ and $\sigma$ as well as in our certainty of their
estimates. In particular we focus on a quantile-based credible
interval for $\alpha$, which is used for determining our confidence in
a gene as being a causal mediator for a trait.
...
of $\widehat{\beta}_j^A$, for $j \in 1,\dots,J$.
The hyperparameters for the priors for $\alpha$ and $\sigma$ are
$\mu_\alpha$, $\textrm{SD}_\alpha$, and $\textrm{SD}_\sigma$.
The first two of these are set based on a simple un-weighted linear model of
...
estimated slope coefficient. $\textrm{SD}_\sigma$ is set by default to
1 (but can be modified by the user), which corresponds to a very wide
